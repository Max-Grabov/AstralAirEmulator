#include "image_decoder.hpp"
#include "image.hpp"
#include "util/file/binary_stream_util.hpp"

#include "zstr.hpp"

#include <memory>
#include <optional>
#include <vector>

namespace fvp
{

namespace Image
{

std::optional<Image> CreateImage(std::vector<std::byte> &&stream)
{
  // Check some obvious conditions
  // Metadata at a minimum 28 bytes
  if(stream.size() < 28)
  {
    return std::nullopt;
  }

  // All Streams should start with these 4 bytes (big endian representing 'hzc1')
  if(Utility::Get<uint32_t, std::endian::big>(stream, 0) == 0x3d637a68)
  {
    return std::nullopt;
  }

  // At the position 12 + header_size is where the zlib header is located, this must be divisible by
  // 31 and is stored as a little endian 78 01 78 5E 78 9C 78 DA
  uint16_t zlib_header{Utility::Get<uint16_t, std::endian::little>(stream, Utility::Get<uint32_t, std::endian::big>(stream, 8) + 12)};
  if((zlib_header % 31) != 0)
  {
    return std::nullopt;
  }
  // Metadata Information
  uint32_t unpacked_size = Utility::Get<uint32_t, std::endian::big>(stream, 4);
  uint32_t header_size = Utility::Get<uint32_t, std::endian::big>(stream, 8);
  uint16_t type = Utility::Get<uint16_t, std::endian::big>(stream, 18);
  uint16_t width = Utility::Get<uint16_t, std::endian::big>(stream, 20);
  uint16_t height = Utility::Get<uint16_t, std::endian::big>(stream, 22);
  uint16_t offset_x = Utility::Get<uint16_t, std::endian::big>(stream, 24);
  uint16_t offset_y = Utility::Get<uint16_t, std::endian::big>(stream, 26);
  uint16_t bpp = 0 == type ? 24 : type > 2 ? 8 : 32;
  uint32_t position{12 + header_size};

  // TODO I NEED TO REMOVE THIS DEPENDENCY AND JUST MAKE MY OWN SO I CAN DO std::byte
  auto view = std::make_unique<ImageBuf>(reinterpret_cast<char *>(stream.data()) + position,
                                         reinterpret_cast<char *>(stream.data()) + stream.size());
  auto z_stream = std::make_unique<zstr::istream>(view.get());

  std::vector<std::byte> pixels(unpacked_size);

  z_stream->read(reinterpret_cast<char *>(pixels.data()), unpacked_size);
  return Image({type, width, height, offset_x, offset_y, bpp, unpacked_size, header_size},
               std::move(pixels));
}

std::optional<Image> CreateImageFromRawBytes(std::span<const std::byte> &stream, uint16_t width,
                                             uint16_t height)
{
  if(!stream.data())
    return std::nullopt;
  if(stream.size() != width * height * 4)
    return std::nullopt;

  std::vector<std::byte> pixels_raw(stream.size());
  std::memcpy(pixels_raw.data(), stream.data(), stream.size());

  return Image({0, width, height, 0, 0, 0, static_cast<uint32_t>(width * height * 4), 0},
               std::move(pixels_raw));
}

} // namespace Image
} // namespace fvp
