...
SDL_AudioStream * CreateAudioStream(const AudioStream &stream)
{
  ...
  // We handle null outside, (just not playing the audio that was going to be inputted via this stream)
  return SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &spec, NULL, NULL); 
}

void PlayAudio(SDL_AudioStream *stream, const AudioStream &data_stream, const std::function<void(SDL_AudioStream *, const AudioStream &)> &data_processing_strategy)
{
  // If no supplied strategy, we just dump all of the data into the stream at once
  if(!data_processing_strategy)
  {
    SDL_PutAudioStreamData(stream, data_stream.GetStream().data(), data_stream.GetStream().size() * sizeof(float)); 
  } 
  else 
  {
